...
public class PurchaseRepository
{
    ...
    private static void ReplaceItems(System.Data.IDbConnection conn, System.Data.IDbTransaction tx, int purchaseId, IEnumerable<PurchaseItem> items)
    {
        foreach (var item in items)
        {
            ...
            conn.Execute(@"INSERT INTO PurchaseItems
                (PurchaseID,ProductID,BatchNumber,ExpiryDate,Quantity,BonusQuantity,PackageQuantity,PurchasePrice,Discount,Tax,ExtraDiscount,ATax)
                VALUES (@PurchaseID,@ProductID,@BatchNumber,@ExpiryDate,@Quantity,@BonusQuantity,@PackageQuantity,@PurchasePrice,@Discount,@Tax,@ExtraDiscount,@ATax)", item, tx);
        }
    }
    ...
    public void PostPurchase(int purchaseId)
    {
        ...
        foreach (var item in items)
        {
            var piecesPerUnit = conn.ExecuteScalar<int>("SELECT ISNULL(NULLIF(PiecesPerUnit,0),1) FROM Products WHERE ProductID=@ProductID", item, tx);
            var stockQuantity = (item.PackageQuantity + item.BonusQuantity) * Math.Max(1, piecesPerUnit);
            conn.Execute(@"UPDATE Products SET Stock=Stock+@Quantity,PurchasePrice=@EffectiveRate,
                LastStockUpdateDate=CAST(GETDATE() AS DATE) WHERE ProductID=@ProductID AND IsActive=1",
                new { Quantity=stockQuantity, EffectiveRate=item.EffectiveRate, item.ProductID }, tx);
        }
