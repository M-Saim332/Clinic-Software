...
| **Reference System** | Ans Medicine — Pharmacy & Inventory Management System *(legacy, UX/functional reference only)* |
| **Target Environment** | Windows Desktop — 2 networked PCs (Doctor + Reception), offline LAN |
| **Technology Stack** | C# (.NET 8+), Avalonia UI, SQL Server Express |
| **Status** | Draft |
| **Version** | 1.0 |
...
2. [Reference System Analysis](#2-reference-system-analysis-ans-medicine--legacy)
3. [New System Requirements](#3-new-system-requirements)
...
This document specifies the new **Clinic Management System**, built to replace informal, manual record-keeping with a desktop application shared between a doctor's PC and a reception PC on the same local network.

The system draws its **functional workflow patterns** — form lifecycle, list/search overlays, master-data registries — from an older reference system, *Ans Medicine*, while adopting a **modern, cross-platform technology stack** and a **simplified, clinic-specific data model** covering patients, medicines, and prescriptions, rather than full pharmacy sales, purchasing, and distributor logistics.

The system must run **fully offline**. The clinic's LAN is available and will be used for PC-to-PC communication, but it is not a substitute for, nor dependent on, internet or cloud connectivity.

---

## 2. Reference System Analysis (Ans Medicine — Legacy)

The legacy system is a pharmacy and inventory management tool. It is **not being ported directly**; it serves only as a source of proven UX patterns.

### 2.1 What We Are Carrying Over

| Pattern | Description |
|---|---|
| **Form lifecycle** | Standardized state machine (below) — a low-error-rate pattern for data-entry staff |
| **List/Find overlay** | Inline searchable grid triggered from any registry form |
| **Master data registries** | Reusable pattern applied to Patients, Medicines, Users (replacing legacy's Company/Product/Customer/Supplier) |
| **Menu-driven navigation** | Simplified for clinic scope |

### 2.2 What We Are Not Carrying Over

- Sales/purchase ledger engine, batch/expiry-driven invoicing, profit & loss reporting, tax/discount computation — these belong to a pharmacy retail/distributor model, not a clinic's patient-and-prescription workflow.
- WinForms + N-Tier legacy architecture — replaced with Avalonia and a simpler layered structure appropriate to this project's scope.
- MDI (Multiple Document Interface) windowing and legacy color-coded modules (amber/green/navy/yellow) — replaced with a single, modern, Fluent-themed interface.

### 2.3 Legacy Form State Machine (Adapted)
...
| **Reports** | Patient list, medicine stock list, expired/low-stock alerts, per-patient visit history |

**Explicitly out of scope** for this version (unlike the legacy pharmacy system): sales invoicing, purchase ledgers, supplier/company management, tax and discount computation, profit & loss reporting. These may be revisited only if the clinic's needs later expand toward retail pharmacy operations.

### 3.2 Non-Functional Requirements

- Must run **fully offline** — no internet dependency.
- Must support **two PCs on the same LAN** reading and writing the same live data.
- Must have a **modern UI**, not the dated legacy WinForms look.
- Must be installable by non-technical staff via a single installer per machine.
- **Zero recurring software cost** — no cloud subscription, no paid licenses.
- Data must survive a single machine failure where reasonably possible (see §6.4, Backup Strategy).
...
- No current requirement for remote access or multi-branch sync — the conditions that would justify cloud don't apply here.
...
- Point the connection string to the server's LAN IP:
...
Since the doctor's PC holds the only copy of the `.mdf` database file, a regular backup routine is required — for example, a scheduled SQL Server backup job to an external drive, or a periodic manual copy. **This is an open item to finalize before go-live.**
...
- The **New / Edit / Delete / Save / Cancel** and **Find / List / Close** action pattern on every registry screen (Patients, Medicines, Users) — a proven, low-error workflow for non-technical staff, restyled rather than redesigned.
...
- [ ] Finalize the backup routine for the doctor's PC database file.
- [ ] Decide on the final installer tool (Inno Setup vs. MSIX vs. WiX).
